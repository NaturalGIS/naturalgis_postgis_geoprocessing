...
package: clean all
	cd .. && rm -f *.zip && zip -r postgis_geoprocessing.zip postgis_geoprocessing -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*Makefile*
	mv ../postgis_geoprocessing.zip .

upload: package
	plugin_uploader.py postgis_geoprocessing.zip
